...
def parse(products):
  ...
  for game in products:
    ...
    results[product_id] = {
      "id": product_id,
      "name": parse_title(game["LocalizedProperties"][0]["ShortTitle"] or game["LocalizedProperties"][0]["ProductTitle"]),
      "desc": f'{release_date} video game by {parse_title(game["LocalizedProperties"][0]["PublisherName"])}'.replace("  ", " "),
